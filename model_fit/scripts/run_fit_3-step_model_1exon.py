...
def nuc_mat_dataset(params, t, delay):
    k2 = np.exp(params['k2'])
    g2 = np.exp(params['g2'])
    t_mod = delayed_time(t, delay)
    return step1(t_mod, k2+g2)

def cyto_dataset(params, t, delay):
    k2 = np.exp(params['k2'])
    g2 = np.exp(params['g2'])
    # k3 = np.exp(params['k3'])
    g3 = np.exp(params['g3'])
    t_mod = delayed_time(t, delay)
    return step2(t_mod, k2+g2, g3)

def mem_dataset(params, t, delay):
    k2 = np.exp(params['k2'])
    g2 = np.exp(params['g2'])
    # k3 = np.exp(params['k3'])
    g3 = np.exp(params['g3'])
    g4 = np.exp(params['g4'])
    t_mod = delayed_time(t, delay)
    return step3(t_mod, k2+g2, g3, g4)
...
def calc_residuals_from_df(params, df, delay):
    ...
    df.loc[:,'function_eval'] = pd.to_numeric(np.select(
        ...
        [
            # nuc_pre_dataset(params, t_array, delay),
            nuc_mat_dataset(params, t_array, delay),
            cyto_dataset(params, t_array, delay),
            # mem_dataset(params, t_array, delay),
        ],
        np.nan
    ))
...
for i in range(samples):
    ...
    #          min=lower_bound, 
    #          max=upper_bound)
    pars.add('g3', value=random_parameter(lower_start_decay, upper_start),
             min=lower_bound_decay, 
             max=upper_bound)
    ...
    pars.add('cyto_over_nuc', expr='exp(k2) / exp(g3)',
    ...
    # --- RUN THE FIT --------------------
    # actual fitting
    out = minimize(objective, pars, args=(T2C_data, overall_delay), 
...
output_path = Path('fit_table_until_cytosol.tsv')
...
error_path = Path('standard_deviation_until_cytosol.tsv')
